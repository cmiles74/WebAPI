...
namespace Nervestaple.WebApi.Controllers {
    /// <summary>
    /// Provides the interface of a read-write controller methods that
    /// can also handle a versioned WebAPI that read-write controllers that
    /// also support a versioned API must implement
    /// </summary>
    public interface IVersionedApiReadWriteController<T, K> : IVersionedApiReadOnlyController<T, K>
        where T: Entity<K> 
        where K: struct 
    {
